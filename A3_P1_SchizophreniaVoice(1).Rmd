...
#mean of the pitch
pitchmean <- lmer(mean ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchmean)

#standard deviation of the pitch
pitchsd<- lmer(stdDev ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchsd)

#range of the pitch
pitchrange<-lmer(range ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchrange)

#median of the pitch
pitchmedian<-lmer(median ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchmedian)

#IQR - interquartile range, a measure of variability, shows where middle 50% of the data is
pitchiqr<-lmer(InterquartileRange ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchiqr)

#Mean Absolute Deviation - the average distance between each data value and the mean
pitchmad<-lmer(MeanAbsoluteDeviation ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchmad)

#Coefficient of variation - the ratio of the standard deviation to the mean (average)
pitchcov<-lmer(coefficientOfVariation ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchcov)

#radius
pitchradius<-lmer(radius ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE)
summary(pitchradius)
...
pitchRR <- lmer(rqa_REC ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE) #check random effects, change fixed effects, etc...
summary(pitchRR)
...
pitchmaxl<-lmer(rqa_maxL ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE) #check random effects, change fixed effects, etc...
summary(pitchmaxl)
...
pitchentr<-lmer(rqa_ENTR ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE) #check random effects, change fixed effects, etc...
summary(pitchentr)
...
pitchtt<- lmer(rqa_TT ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE) #check random effects, change fixed effects, etc...
summary(pitchtt)
...
pitchlam<-lmer(rqa_LAM ~ diagnosis+trial+study+(1|participant), pitch, REML = FALSE) #check random effects, change fixed effects, etc...
summary(pitchlam)
