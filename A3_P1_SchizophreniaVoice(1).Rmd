...
pitch2<-read.csv("final_rqa.csv",header = T)


#RR - recurrence rate, high rr- lots of similar values in the timeseries, low rr - the timeseries contains many different values
pitchRR <- lmer(P_RR ~ Diagnosis + Trial + Study +(1+Trial|Subject), pitch, REML = FALSE) #check random effects, change fixed effects, etc...
summary(pitchRR)

#mean of the pitch
pitchmean <- lmer(P_Mean ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchmean)

#standard deviation of the pitch
pitchsd<- lmer(P_SD ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchsd)

#range of the pitch
pitchrange<-lmer(P_Range ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchrange)

#median of the pitch
pitchmedian<-lmer(P_Median ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchmedian)

#IQR - interquartile range, a measure of variability, shows where middle 50% of the data is
pitchiqr<-lmer(P_IQR ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchiqr)

#Mean Absolute Deviation - the average distance between each data value and the mean
pitchmad<-lmer(P_MeanAbDif ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchmad)

#Coefficient of variation - the ratio of the standard deviation to the mean (average)
pitchcov<-lmer(P_CovofVar ~ Diagnosis+Trial+Study+(1+Trial|Subject), pitch, REML = FALSE)
summary(pitchcov)
